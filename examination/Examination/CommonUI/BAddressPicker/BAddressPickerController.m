...
- (BOOL)searchBarShouldBeginEditing:(UISearchBar *)searchBar{
    if ([self.delegate respondsToSelector:@selector(addressBeginFind:)]) {
        [self.delegate addressBeginFind:searchBar];
...
- (BOOL)searchBarShouldEndEditing:(UISearchBar *)searchBar{
    if ([self.delegate respondsToSelector:@selector(addressEndFind:)]) {
        [self.delegate addressEndFind:searchBar];
        if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 7.0) {
            [UIView animateWithDuration:0.25 animations:^{
                for (UIView *subview in self.view.subviews){
...
//        NSString *path = [[NSBundle mainBundle] pathForResource:@"cities" ofType:@"plist"];
        ...
        //检查是否写入
        _dictionary = [NSMutableDictionary dictionaryWithContentsOfFile:fileName];
    }
    return _dictionary;
}
