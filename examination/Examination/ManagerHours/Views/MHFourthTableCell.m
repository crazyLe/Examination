...
@implementation MHFourthTableCell
...
{
    if (field.tag == 100) {
        if (_delegate && [_delegate respondsToSelector:@selector(MHFourthTableCellTF:withTextfield:)]) {
            [_delegate MHFourthTableCellTF:@"按时收费" withTextfield:field];
        }
    }else{
        if (_delegate && [_delegate respondsToSelector:@selector(MHFourthTableCellTF:withTextfield:)]) {
            [_delegate MHFourthTableCellTF:@"按圈收费" withTextfield:field];
        }
    }
}
...
- (IBAction)clickFirstBtn:(id)sender {
    ...
    [_secondBtn setImage:[UIImage imageNamed:@"manager_gray1.png"] forState:UIControlStateNormal];
    _field2.text = @"";
    if (_delegate && [_delegate respondsToSelector:@selector(MHFourthTableCellClickBtn:)]) {
        [_delegate MHFourthTableCellClickBtn:@"按时收费"];
    }
}
- (IBAction)clickSecondBtn:(id)sender {
    ...
    _field1.text = @"";
    if (_delegate && [_delegate respondsToSelector:@selector(MHFourthTableCellClickBtn:)]) {
        [_delegate MHFourthTableCellClickBtn:@"按圈收费"];
    }
}
